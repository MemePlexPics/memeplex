...
export const BlaclListSettings = (props: { password: string; className?: string }) => {
  const { t } = useTranslation()
  const setNotification = useNotification()
  const [blacklist, setBlacklist] = useState('')
  const { handleAdminRequest } = useAdminRequest()
  const request = useFetch<{ words: string }>(() => getUrl('/blacklist', {}), {
    deps: [],
  })
  ...
  return (
    <div {...stylex.props(s.blacklist)}>
      ...
      />
      <Button onClick={onClickSubmit}>{t('button.submit')}</Button>
    </div>
  )
}
