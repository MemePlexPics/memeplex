...
import { selectBotSubscriptionsByKeywords, selectBotTopicKeywordUnsubscriptions, selectBotTopicWithKeywords } from '../../utils/mysql-queries'
...
describe('processMessages', () => {
  ...
  const mockGetPublisherUserByChannelIdAndTariffPlan = jest.mocked(getPublisherUserByChannelIdAndTariffPlan)
  ...
  const setupMocks = async (mockValues: {
    ...
    mockGetAmqpQueue.mockResolvedValue([{
      ...
      ack: jest.fn(),
      close: jest.fn(),
    } as unknown as amqplib.Channel, jest.fn(), jest.fn()])
    ...
    mockGetPublisherUserByChannelIdAndTariffPlan.mockImplementation(async (db, queue, tariffPlanByUsers, channelId, memeId, memeData) => {
  ...
  test('Should process messages correctly', async () => {
    await setupMocks({
      ...
      topics: [
        { id: 1, nameId: 1, keywordId: 1, keyword: 'keyword1' },
        { id: 2, nameId: 2, keywordId: 2, keyword: 'keyword2' },
      ],
      subscriptions: [
        { channelId: 102, keyword: 'keyword2' }
      ],
      unsubscriptions: [
        { channelId: 101, keyword: 'keyword1' }
      ],
      topicSubscriptions: {
    ...
    expect(mockGetAmqpQueue).toHaveBeenCalledWith(expect.any(Object), process.env.AMQP_NLP_TO_PUBLISHER_CHANNEL + '_test')
  ...
  test('Should process messages with different mock values', async () => {
    await setupMocks({
      ...
      topics: [
        { id: 3, nameId: 3, keywordId: 3, keyword: 'keyword3' },
      ],
      subscriptions: [],
      unsubscriptions: [],
      topicSubscriptions: {
