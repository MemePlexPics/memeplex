...
import { botKeywords, botTopicKeywordUnsubscriptions, botTopicNames, botTopicSubscriptions, botTopics } from "../db/schema"
  ...
  for (const topic of topicsInDb) {
    if (!topics[topic.name]) {
      ...
      await db
        ...
        .where(eq(botTopicSubscriptions.topicId, topic.id))
      const keywordsFromTopic = await db.select().from(botTopics).where(eq(botTopics.nameId, topic.id))
      const keywordIdsFromTopic = keywordsFromTopic.map(keyword => keyword.keywordId)
      // Drop keywords without reference
      try {
        await db
          .delete(botKeywords)
          .where(inArray(botKeywords.id, keywordIdsFromTopic))
      } catch (error) {
      ...
      // Drop topic
      await db
        .delete(botTopicNames)
        .where(eq(botTopicNames.name, topic.name))
      continue
    }
    ...
    const keywordsToDelete = oldTopicKeywordsInDb.filter(oldKeyword => !topics[topic.name].includes(oldKeyword.keyword))
    ...
    await db.insert(botKeywords).ignore().values(topics[topic.name].map(keyword => ({ keyword })))
    const keywordsInDb = await selectBotKeywordsByKeywords(db, topics[topic.name])
    // Insert topic-keyword connections
    await db.insert(botTopics).ignore().values(keywordsInDb.map(keyword => ({
